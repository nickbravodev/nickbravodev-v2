...
import { A as AstroError, U as UnknownContentCollectionError, a as createComponent, j as RenderUndefinedEntryError, u as unescapeHTML, d as renderTemplate, G as GetEntryDeprecationError, k as renderUniqueStylesheet, l as renderScriptElement, n as createHeadAndContent, r as renderComponent } from './astro/server_tDdmYNVf.mjs';
import 'kleur/colors';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_Byqmhxaw.mjs');
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (collection in contentCollectionToEntryMap) {
      ...
      const { default: imageAssetMap } = await import('./content-assets_CiZd6orn.mjs');
      const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_BeanF8c5.mjs').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules_DnLIV65O.mjs');
